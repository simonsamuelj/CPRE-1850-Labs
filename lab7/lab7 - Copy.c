...
int main(int argc, char* argv[]) {
	...
	// Stop when square is pressed
	Button_S = 0;

    while (scanf("%lf %lf %lf %d", &x, &y, &z, &Button_T, &Button_C, &Button_X, &Button_S) == 4 && !Button_S) {
        ...
        if (count < lengthofavg) {
            ...
            printf("Value(x,y,z): %lf, %lf, %lf, Average(x,y,z): %lf, %lf, %lf, Extrema(x): %lf, %lf, Extrema(y): %lf, %lf, Extrema(z): %lf, %lf\n", x, y, z, xav, yav, zav, xmax, xmin, ymax, ymin, zmax, zmin);
        }
    }
